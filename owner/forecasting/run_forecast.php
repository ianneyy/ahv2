...
<body>
    ...
    if ($eval_result && $eval_result->num_rows > 0) {
        ...
        foreach ($data as $crop => $years) {
            ...
            if ($overall_winner == 'Prophet')
                echo " <span class='winner-badge'>WINNER</span>";
            ...
            foreach ($years as $year => $methods) {
                ...
                if (isset($methods['Prophet'])) {
                    ...
                    if ($is_winner)
                        echo "<span class='winner-badge'>BEST FOR {$year}</span>";
                    echo "</h4>";

                    $mape_class = 'good-accuracy';
                    if ($m['mape'] > 20)
                        $mape_class = 'poor-accuracy';
                    elseif ($m['mape'] > 10)
                        $mape_class = 'medium-accuracy';
    ...
    <div
        style='background: #e8f5e9; padding: 20px; border-left: 4px solid #4CAF50; margin-top: 40px; border-radius: 4px;'>
        ...
        <p><strong>RMSE (Root Mean Squared Error):</strong> Lower is better. Measures average prediction error in
            pieces.</p>
